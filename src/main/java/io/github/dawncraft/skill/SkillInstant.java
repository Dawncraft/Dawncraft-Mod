...
public class SkillInstant extends Skill
{
    ...
    public EnumActionResult onSkillSpell(SkillStack skillStack, World world, EntityPlayer player)
    {
        if (!world.isRemote)
        {
            if (this == SkillInit.HEAL)
            {
                player.heal(4.0F + 2.0F * skillStack.getLevel());
            }
            else if (this == SkillInit.ATTACK)
            {
                player.attackEntityFrom(DamageSourceLoader.causeSkillDamage(skillStack, player), 2.0F + 1.0F * skillStack.getLevel());
            }
        ...
        world.spawnParticle(ParticleInit.LIGHTING, player.getPosition().getX(), player.getPosition().getY(), player.getPosition().getZ(), 0.0D, 0.0D, 0.0D);
